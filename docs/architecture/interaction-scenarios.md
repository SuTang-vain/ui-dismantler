...
可以根据 analyzer manifest 生成待审阅候选：

```bash
python3 scripts/generate_scenarios.py manifest.json --out candidates.json
```

候选场景包含 `candidate: true`、`covers` fingerprint 和人工复核 notes。
它们可以被协议加载和执行，但默认不计入交互覆盖率；确认 selector、补齐能证明状态
真实发生的 assertions 后，移除 `candidate` 字段，才会进入覆盖率统计。

将 manifest 与正式场景接入 roundtrip：
