...
- `source-readiness.ts` rejects unresolved runtime-only HTML shells, missing critical local assets, and unreviewed WebGL inputs before materialization; frozen local runtime resources are accepted as inspectable source evidence, and Canvas responsibility requires an explicit reviewed resource profile;
- `acceptance.ts` is the formal release boundary for agent-authored or externally materialized libraries that do not originate from a Build Plan; it hashes the source, library (excluding `.ui-dismantler`, `.git`, and `node_modules`), fixed quality contract, and quality result, then writes `.ui-dismantler/quality-report.json` and `.ui-dismantler/acceptance-receipt.json`;
- `pipeline.ts` writes `ComponentLibraryBuildReport` plus `.ui-dismantler/production-receipt.json`, and invokes existing static validation plus reviewed multi-viewport/browser execution settings for the existing quality gates; only `accepted` is releasable, while missing reviewed quality evidence is `review-required`; quality thresholds are not overridable through the component production contract; browser processes retain an auditable `BrowserServer` owner and bounded graceful shutdown with locked fast fallback;
