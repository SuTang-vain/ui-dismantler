...
- Feature Tabs：点击切换，激活 Tab 使用 `aria-selected=true`，面板使用 `role=tabpanel`。
- 键盘：聚焦 Tab 后使用 `ArrowLeft` / `ArrowRight` 循环切换。
- FAQ：使用原生 `<details>` / `<summary>` 展开收起。
- CTA：邮箱输入仅管理本地输入状态，提交接口由宿主应用注入。

## 组件

组件以 `Mastra.mount(container, options)` 和 `Mastra.create(options)` 作为稳定入口；`template.html` 是默认数据空模板，`mastra.html` 是带完整案例数据的展示示例。
