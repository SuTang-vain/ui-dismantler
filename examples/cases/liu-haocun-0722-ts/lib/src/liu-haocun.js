  ...
  var TEMPLATE = `<div id="sg-app-frame">
    <div id="sg-app-container" aria-label="刘浩存演员动态百科">
      <main class="sg-stage">
        ...
        <section class="sg-view sg-works-view" id="sg-view-works" aria-label="参演作品"></section>
      ...
      <nav class="sg-nav" aria-label="底部导航">
        ...
        <button class="sg-nav-btn" type="button" data-view="works">
          参演作品
        </button>
  ...
  function mount(root, options) {
    ...
    root.innerHTML = TEMPLATE;
          ...
          (function () {
            ...
            var works = (options && options.works) || [
              {
                ...
                posters: ["../assets/七根心简_海报.jpg", "../assets/qigen-2.webp", "../assets/qigen-3.webp"],
                release: "2025播出",
                stat: "奇幻冒险 · 角色突破",
              ...
              },
              {
                ...
                posters: ["../assets/脱轨_海报.jpg", "../assets/tuogui-2.webp", "../assets/tuogui-3.webp"],
                release: "2023播出",
                stat: "都市奇幻 · 反差成长",
              ...
              },
              {
                ...
                posters: ["../assets/灿烂的她海报.jpeg", "../assets/canlan-2.webp", "../assets/canlan-3.webp"],
                release: "2024上映",
                stat: "亲情题材 · 双女主陪伴",
                desc: "亲情题材电影，围绕重逢与陪伴。",
              },
              {
                ...
                posters: ["../assets/念念相忘_海报.jpg", "../assets/niannian-2.webp", "../assets/niannian-3.webp"],
                release: "2023上映",
                stat: "青春爱情 · 校园记忆",
                desc: "青春爱情电影，主打校园记忆。",
              },
              {
                ...
                posters: ["../assets/龙马精神海报.jpeg", "../assets/longma-2.webp", "../assets/longma-3.webp"],
                release: "2023上映",
                stat: "动作喜剧 · 亲情陪伴",
                desc: "动作喜剧电影，兼具亲情与陪伴。",
              },
              {
                ...
                posters: ["../assets/四海_海报.jpg", "../assets/sihai-2.webp", "../assets/sihai-3.webp"],
                release: "2022上映",
                stat: "青春公路 · 自由气质",
              ...
              },
              {
                ...
                posters: ["../assets/送你一朵小红花_海报.jpg", "../assets/honghua-2.webp", "../assets/honghua-3.webp"],
                release: "2020上映",
                stat: "金鸡奖最佳女主角提名",
              ...
              },
              {
                ...
                posters: ["../assets/一秒钟_海报.jpeg", "../assets/yimiao-2.webp", "../assets/yimiao-3.webp"],
                release: "2020上映",
                stat: "亚洲电影大奖最佳新演员",
            ...
            var viewWorks = document.getElementById("sg-view-works");
            ...
            var workCategory = "电影";
            var activeWorkIds = { "电影": "canlan", "剧集": "qigen" };
            var activePosterIndexes = {};
            var visitedWorkIds = {};
            var worksInteracted = false;
            var timer = null;
            var startX = 0;
            ...
            function renderWorks() {
              var categories = ["电影", "剧集"];
              var list = visibleWorks();
              var active = activeWork();
              var visitedCount = Object.keys(visitedWorkIds).filter(function (id) {
                return visitedWorkIds[id];
              }).length;
              var next = nextWorkInCategory(active.id);
              var progress = Math.round((visitedCount / works.length) * 100);
              var progressCopy = !worksInteracted
                ? "选一部，看她的角色变化"
                : visitedCount === works.length
                  ? "作品图鉴已全部探索"
                  : "下一站 · " + next.title;
              viewWorks.innerHTML =
                "<div class=\"sg-works-toolbar\">" +
                  "<div class=\"sg-category-row\" aria-label=\"作品类型\">" +
                  categories
                    .map(function (cat) {
                      var count = works.filter(function (work) {
                        return work.category === cat;
                      }).length;
                      return (
                        "<button class=\"sg-category-btn" +
                        (cat === workCategory ? " sg-active" : "") +
                        '" type="button" data-cat="' +
                        escapeHtml(cat) +
                        '" aria-pressed="' +
                        (cat === workCategory ? "true" : "false") +
                        '">' +
                        escapeHtml(cat) +
                        " " +
                        count +
                    ...
                    .join("") +
                  "</div>" +
                  "<div class=\"sg-works-progress\" role=\"progressbar\" aria-label=\"作品探索进度\" aria-valuemin=\"0\" aria-valuemax=\"" +
                  works.length +
                  '" aria-valuenow="' +
                  visitedCount +
                  '">' +
                    "<span class=\"sg-works-progress-copy\">" + escapeHtml(progressCopy) + "</span>" +
                    "<b class=\"sg-works-progress-count\">已探索 " + visitedCount + "/" + works.length + "</b>" +
                    "<span class=\"sg-works-progress-track\"><i style=\"width:" + progress + '%"></i></span>' +
                  "</div>" +
                "</div>" +
                workFeature(active, next) +
                "<div class=\"sg-work-strip-shell\">" +
                  "<div class=\"sg-work-strip\" aria-label=\"" +
                  escapeHtml(workCategory) +
                  '片单">' +
                  list
                    .map(function (work, index) {
                      return workThumb(work, index, active.id);
                    })
                    .join("") +
                  "</div>" +
                "</div>";
              window.setTimeout(function () {
                var selected = viewWorks.querySelector(".sg-work-thumb[aria-pressed=\"true\"]");
            ...
            function activeWork() {
              var list = visibleWorks();
              var id = activeWorkIds[workCategory];
              var found = list.filter(function (work) {
                return work.id === id;
              })[0];
              if (!found) {
                found = list[0];
                activeWorkIds[workCategory] = found.id;
              }
              return found;
            }
    
            function nextWorkInCategory(currentId) {
              var list = visibleWorks();
              var currentIndex = list.findIndex(function (work) {
            ...
            function markWorkVisited(id) {
              visitedWorkIds[id] = true;
              worksInteracted = true;
            }
    
            function selectWork(id, markVisited, restoreFocus) {
              var work = workById(id);
              workCategory = work.category;
              activeWorkIds[work.category] = work.id;
              if (markVisited) markWorkVisited(work.id);
              renderWorks();
              if (restoreFocus) {
                window.setTimeout(function () {
                  var button = viewWorks.querySelector('[data-select-work="' + work.id + '"]');
                  if (button) button.focus();
                }, 0);
              }
            }
    
            function goToNextWork(restoreFocus) {
              var current = activeWork();
              markWorkVisited(current.id);
              var next = nextWorkInCategory(current.id);
              selectWork(next.id, false, false);
              if (restoreFocus) {
                window.setTimeout(function () {
                  var button = viewWorks.querySelector("[data-next-work]");
                  if (button) button.focus();
                }, 0);
              }
            }
    
            function selectPoster(workId, index, restoreFocus) {
              var work = workById(workId);
              if (!work || !work.posters || !work.posters[index]) return;
              activePosterIndexes[work.id] = index;
              markWorkVisited(work.id);
              renderWorks();
              if (restoreFocus) {
                window.setTimeout(function () {
                  var button = viewWorks.querySelector(
                    '[data-select-poster="' + index + '"]',
                  );
                  if (button) button.focus();
                }, 0);
              }
            ...
            function workFeature(work, next) {
              var posters = work.posters && work.posters.length ? work.posters : [work.poster];
              var activePosterIndex = activePosterIndexes[work.id] || 0;
              if (!posters[activePosterIndex]) activePosterIndex = 0;
              var leftPosterIndex = (activePosterIndex + posters.length - 1) % posters.length;
              var posterCards = posters
                .map(function (poster, index) {
                  var positionClass = index === activePosterIndex
                    ? "sg-is-active"
                    : index === leftPosterIndex
                      ? "sg-is-left"
                      : "sg-is-right";
                  return (
                    "<button class=\"sg-feature-poster-card " +
                    positionClass +
                    '" type="button" data-select-poster="' +
                    index +
                    '" aria-pressed="' +
                    (index === activePosterIndex ? "true" : "false") +
                    '" aria-label="切换至' +
                    escapeHtml(work.title) +
                    '第' +
                    (index + 1) +
                ...
                .join("");
              return (
                "<article class=\"sg-work-feature\" data-active-work=\"" +
                escapeHtml(work.id) +
                '">' +
                "<div class=\"sg-feature-poster\">" +
                  "<span class=\"sg-feature-watermark\" aria-hidden=\"true\">" +
                  escapeHtml(work.title) +
                  "</span>" +
                  "<div class=\"sg-feature-poster-gallery\">" +
                  posterCards +
                  "</div>" +
                  "<span class=\"sg-feature-status\">" +
                  escapeHtml(work.status) +
                  "</span>" +
                "</div>" +
                "<div class=\"sg-feature-info\" aria-live=\"polite\">" +
                "<div class=\"sg-feature-heading\">" +
                  "<span class=\"sg-feature-kicker\">" +
                  escapeHtml(work.release) +
                  " · " +
                  escapeHtml(work.category) +
                  "</span>" +
                  "<h2 class=\"sg-feature-title\">" +
                  escapeHtml(work.title) +
                  "</h2>" +
                "</div>" +
                "<p class=\"sg-feature-desc\">" + escapeHtml(work.desc) + "</p>" +
                "<div class=\"sg-feature-facts\" aria-label=\"角色档案\">" +
                  "<span class=\"sg-feature-fact\"><small>饰演角色</small><b>" + escapeHtml(work.role) + "</b></span>" +
                  "<span class=\"sg-feature-fact\"><small>合作演员</small><b>" + escapeHtml(work.partners) + "</b></span>" +
                  "<span class=\"sg-feature-fact\"><small>作品看点</small><b>" + escapeHtml(work.stat) + "</b></span>" +
                "</div>" +
                "<div class=\"sg-feature-actions\">" +
                  "<button class=\"sg-feature-next-button\" type=\"button\" data-next-work aria-label=\"下一部：" +
            ...
            function workThumb(work, index, activeId) {
              var visited = Boolean(visitedWorkIds[work.id]);
              var isActive = work.id === activeId;
              var hint = !worksInteracted && !isActive && index === 1;
              return (
                "<button class=\"sg-work-thumb" +
                (isActive ? " sg-active" : "") +
                (visited ? " sg-visited" : "") +
                (hint ? " sg-is-hint" : "") +
                '" type="button" data-select-work="' +
                escapeHtml(work.id) +
                '" aria-pressed="' +
                (isActive ? "true" : "false") +
                '" aria-label="查看' +
                ...
                (visited ? "，已探索" : "，未探索") +
                '">' +
                  "<span class=\"sg-work-thumb-poster\"><img src=\"" +
                  escapeHtml(work.poster) +
                  '" alt=""></span>' +
                  "<span class=\"sg-work-thumb-copy\">" +
                    "<span class=\"sg-work-thumb-title\">" + escapeHtml(work.title) + "</span>" +
                    "<span class=\"sg-work-thumb-year\">" + escapeHtml(work.release) + "</span>" +
                    "<span class=\"sg-work-thumb-state\">" +
                    (isActive
                      ? "正在看"
                      : visited
                        ? "饰 " + escapeHtml(compactRole(work.role))
                        : "看她演谁") +
                    "</span>" +
                  "</span>" +
                "</button>"
            ...
            function bindEvents() {
              ...
              viewWorks.addEventListener("click", function (event) {
                var cat = event.target.closest(".sg-category-btn");
                if (cat) {
                  workCategory = cat.getAttribute("data-cat");
                  renderWorks();
                  if (event.detail === 0) {
                    window.setTimeout(function () {
                      var activeCategory = viewWorks.querySelector(
                        '[data-cat="' + workCategory + '"]',
                      );
                ...
                var posterButton = event.target.closest("[data-select-poster]");
                if (posterButton) {
                  var activeArticle = posterButton.closest("[data-active-work]");
                  if (activeArticle) {
                    selectPoster(
                      activeArticle.getAttribute("data-active-work"),
                      Number(posterButton.getAttribute("data-select-poster")) || 0,
                      event.detail === 0,
                    );
                  }
                  return;
                }
                var workButton = event.target.closest("[data-select-work]");
                if (workButton) {
                  selectWork(
                    workButton.getAttribute("data-select-work"),
                    true,
                    event.detail === 0,
                  );
                  return;
                }
                var nextButton = event.target.closest("[data-next-work]");
                if (nextButton) {
                  goToNextWork(event.detail === 0);
                  return;
                }
              });
