...
> 从 `BLACKPINK v10 最新迭代版` 案例页提炼的可复用组件库。一套数据驱动的"明星组合"垂类卡片，覆盖 PC / WISE / 极端小屏三档响应式，内置**初始引导遮罩**、成员网格自动播放、时间线原地展开故事、3D 中心聚焦作品轮播、创作故事面板、资料 Modal 等完整交互。
...
└── examples/
    ├── blackpink.html        ← 用组件库 + 原数据复刻原 BLACKPINK v10 最新版案例
...
```ts
{
  ...
  entryCoverImg: string,         // 初始引导遮罩的组合照 URL（铺满画布）
  entryCoverAlt: string,         // 组合照 alt 文本
  entryCells: [                  // 与 tabs 对齐的玻璃入口（4 项）
    { no: '01', name: '成员详情', arrow: '↓' },
    { no: '02', name: '经历',     arrow: '↓' },
    { no: '03', name: '团体作品', arrow: '↓' },
    { no: '04', name: '其它',     arrow: '↗' }
  ],
  members: [
    {
...
完整变量清单见 `docs/设计规范.md` 第一节（11 个主题色令牌 + 6 个派生令牌）。
...
| 组件 | 类名前缀 | 说明 |
|---|---|---|
| 画布 | `.sg-frame` | 固定尺寸 + 三档响应式 + isolation 层叠上下文 |
| Tab Bar | `.sg-tab-bar` / `.sg-tab` | N 等分，末位 `.sg-tab-more` 特殊态 |
| 视图栈 | `.sg-view-stack` / `.sg-view` | 绝对定位堆叠，显隐切换 |
| 初始引导遮罩 | `.sg-entry-cover` / `.sg-entry-cell` | 组合照玻璃磨砂四分入口 + 淡出动效（v1.2 新增） |
...
| Modal | `.sg-modal-overlay` / `.sg-modal-card` | 事实网格 + 统一 X 按钮 + 成员详情（含头像信息头） |
...
| 模块 | 触发 | 行为 |
|---|---|---|
| 初始引导遮罩 | 页面加载 | 组合照玻璃磨砂四分入口覆盖整个画布 |
| 引导玻璃块（前三个） | 点击 | 进入对应 Tab + 淡出遮罩 |
| 引导玻璃块（其它） | 点击 | 恢复初始态 + 显示遮罩 + 打开资料 Modal |
| Tab 切换（前三个） | 点击 | 切换面板 + 重置时间线 + 启停作品自动播放 + 淡出遮罩 |
| Tab 切换（其它） | 点击 | 恢复初始态 + 显示遮罩 + 打开 Modal |
...
| Modal | "其它"Tab/玻璃块 | 打开资料说明；ESC/点遮罩/X 关闭 |
...
| 断点 | 画布 | 关键差异 |
|---|---|---|
| PC（默认） | 788×492 | 双栏、时间线 3 列/页 + 原地展开 58% 宽、作品 5 档位、引导遮罩玻璃块完整尺寸 |
| `≤500px`（WISE） | 380×456 | 单栏、详情面板隐藏、时间线 2 列 + 底部控制栏 + 展开 82%、引导遮罩玻璃块字号缩小 |
| `≤320px 或 ≤380h`（极端） | 100%（min 280×340） | 隐藏 `<small>` 计数、点击成员弹 Modal、时间线 2 列 + 展开 88%、引导遮罩字号再缩小 |

---

## 与 gold-standard（v1.1）的差异

- **初始引导遮罩**：新增 `.sg-entry-cover` + `.sg-entry-cell`。组合照玻璃磨砂四分入口，点击玻璃块进入对应 Tab 并淡出遮罩；点「其它」恢复初始态叠加弹窗。默认入口未选中任何 Tab，需用户主动点 Tab 或玻璃块进入视图。
- **主题色令牌**：完整收录实际 HTML 的 11 个令牌（含 `--sg-soft-accent` 即原 `--soft-pink`、`--sg-accent-2` 即原 `--accent-2`）。
- **画布层级**：`.sg-frame` 加 `isolation: isolate`，确保引导遮罩的 z-index 限制在卡片内、body 级弹窗始终位于整张卡片之上。
- **Modal 数据契约**：新增 `moreModalDecl`（资料 Modal 底栏声明，默认与 `memberModalDecl` 同值）。
...
- v1.2.0 - 从 BLACKPINK v10 最新迭代版案例提炼，新增初始引导遮罩特性（组合照玻璃磨砂四分入口）
- v1.1.0 - 从 BLACKPINK v10 案例提炼（gold-standard），新增时间线原地展开故事特性
