...
(() => {
  ...
  const bind = () => {
    app.querySelectorAll('.navigation-links a').forEach(link => link.addEventListener('click', event => {
      ...
      router.navigate(href, { source: 'auto-router' });
    }));
    app.querySelector('[data-history="back"]')?.addEventListener('click', () => router.back());
    app.querySelector('[data-history="forward"]')?.addEventListener('click', () => router.forward());
    app.querySelector('input[type="search"]')?.addEventListener('keydown', event => {
      if (event.key === 'Enter') router.navigate(`/search/${encodeURIComponent(event.currentTarget.value)}`, { source: 'auto-router' });
    });
  };
  const render = () => { app.innerHTML = `${renderNav()}<main>${renderRoute()}</main>`; bind(); };
  history.replaceState({ source: 'auto-router', route: routePath() }, '', routePath());
