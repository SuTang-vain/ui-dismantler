"""Library validation: baseline constraints plus optional Gold delivery checks.

基础档检查命名、变量、数据分离、响应式、A11y、主题、零依赖、交付物和
CSS/JS 类名对齐；Gold 档继续检查稳定 API、完整文档和 Bento Showcase。
...
# ============================================================
class LibValidator:
    def __init__(self, lib_dir: str, *, require_showcase: bool = False, quality_profile: str = "baseline"):
        self.dir = Path(lib_dir).resolve()
        self.require_showcase = require_showcase
    ...
    def check_docs(self):
        ...
        template = self.dir / "examples" / "template.html"
        showcase = self.dir / "showcase.html"
        ...
        # template.html is part of the stable component-library contract, not an optional example.
        if not template.exists():
            issues.append("缺少 examples/template.html 空复用模板")
        elif "mount" not in template.read_text(encoding="utf-8", errors="replace"):
            issues.append("examples/template.html 缺少 mount 挂载示例")
        # showcase is generated by the separate showcase CLI. Keep the legacy validator compatible
        # by checking it only when the complete-delivery gate is explicitly requested.
        if self.require_showcase:
            if not showcase.exists():
                issues.append("缺少 showcase.html（请先运行 generate_showcase.py）")
            elif "<!doctype html>" not in showcase.read_text(encoding="utf-8", errors="replace").lower():
                issues.append("showcase.html 不是合法的 HTML 展示页")
        detail = "；".join(issues) if issues else "README.md + docs/设计规范.md + examples/template.html 齐全"
        if not issues and self.require_showcase:
            detail += " + showcase.html 齐全"
        self.record("8. 文档与交付物完备", not issues, detail)

    # ---------- 9. 类名对齐 ----------
    def check_class_alignment(self):
        """第 9 项：JS 引用的 sg-* 类名必须在 CSS 中有定义（拦截孤儿类导致元素无样式）。

        - 从 CSS 全部规则（含 @media 内）提取所有 .sg-* 类名集合 ``CSS_DEFINED``
        - 从 JS 提取作为 class 使用的 sg-* 类名（el() 调用 + class= 字面量 + querySelector(".sg-xxx")）
        - 报告 ``JS_USED - CSS_DEFINED``（JS 用了但 CSS 没定义 = 元素无样式）
        - 语义基类（如 sg-tl-next/sg-tl-prev 用于组合选择器，CSS 只定义带后缀的变体）不报
        """
        issues = []
        # 1. 提取 CSS 定义的所有 .sg-* 类名（全量选择器，不只主体）
        css_classes: set[str] = set()
        for sel, _ in parse_rules(self.css):
            for cls in re.findall(r"\.(sg-[\w-]+)", sel):
                css_classes.add(cls)
        # 2. 提取 JS 中作为 class 使用的 sg-* 类名
        #    覆盖三种载体：el(tag, 'cls')、class="cls"、querySelector('.cls')
        #    避免抓取 id="sg-xxx"、--sg-xxx 变量名、注释里的描述性文本
        js_class_patterns = [
            r"""el\(\s*['"][a-z]+['"]\s*,\s*['"]([^'"]*sg-[\w-]+[^'"]*)['"]""",  # el('tag', 'cls')
            r"""class\s*=\s*['"]([^'"]*sg-[\w-]+[^'"]*)['"]""",                 # class="cls"
            r"""classList\.(?:add|remove|toggle)\(\s*['"]([^'"]*)['"]""",       # classList.add('cls')
            r"""querySelector(?:All)?\(\s*['"]\.([^'".\s]+)['"]""",             # querySelector('.cls')
            r"""className\s*[:=]\s*['"]([^'"]*sg-[\w-]+[^'"]*)['"]""",          # className: 'cls'
            r"""setAttribute\(\s*['"]class['"]\s*,\s*['"]([^'"]*)['"]""",        # setAttribute('class', 'cls')
        ]
        js_classes: set[str] = set()
        for pattern in js_class_patterns:
            for match in re.finditer(pattern, self.js):
                for token in re.findall(r"(sg-[a-z][\w-]*)", match.group(1)):
                    js_classes.add(token)
        # 3. 差集：JS 用了但 CSS 没定义
        # 豁免：动态拼接的 ID 前缀（'sg-tab-' + tabId 等），不是 CSS 类名
        # 豁免：语义基类用于组合选择器（CSS 定义带后缀的变体，基类用于 querySelector）
        dynamic_id_prefixes = (
            "sg-tab-", "sg-panel-", "sg-rel-", "sg-mdm-",
            "sg-member-modal-", "sg-tl-page-label", "sg-work-story-",
        )
        # 语义基类：JS 用作组合类名的一部分（'sg-arrow sg-prev'）或 querySelector 锚点，
        # CSS 定义带后缀的变体（sg-tl-prev-pc / sg-tl-prev-mobile）或不单独定义（sg-arrow/sg-prev/sg-next）
        semantic_base_classes = {
            "sg-arrow", "sg-prev", "sg-next", "sg-dots", "sg-dot",
            "sg-tl-prev", "sg-tl-next", "sg-tl-dot", "sg-tl-dots",
        }
        def has_css_variant(class_name: str) -> bool:
            return any(candidate.startswith(class_name + "-") for candidate in css_classes)

        orphans = sorted(
            cls for cls in (js_classes - css_classes)
            if not cls.startswith(dynamic_id_prefixes)
            and not (cls in semantic_base_classes and has_css_variant(cls))
        )
        for cls in orphans:
            issues.append(f"JS 引用 .{cls} 但 CSS 未定义（元素将无样式）")
        detail = "；".join(issues[:5]) if issues else "JS 引用的 sg-* 类名均有 CSS 契约"
        self.record("9. 类名对齐", not issues, detail)

    # ---------- 10. Gold 质量契约（完整交付时启用） ----------
    def check_gold_quality(self):
        ...
        spec_text = spec.read_text(encoding="utf-8", errors="replace") if spec.exists() else ""
        for keyword in ("API", "数据契约", "主题"):
            if keyword not in readme_text:
                issues.append(f"README.md 缺少 {keyword} 章节")
        for keyword in ("主题色", "交互", "响应式", "A11y", "组件"):
            if keyword not in spec_text:
                issues.append(f"设计规范.md 缺少 {keyword} 内容")

        if self.require_showcase:
            showcase = self.dir / "showcase.html"
            showcase_text = showcase.read_text(encoding="utf-8", errors="replace") if showcase.exists() else ""
            for marker in ('id="overview"', 'id="colors"', 'id="components"', 'id="breakpoints"', 'ds-bento-grid'):
                if marker not in showcase_text:
                    issues.append(f"showcase.html 缺少高密度展示结构 {marker}")

        detail = "；".join(issues[:6]) if issues else "mount/create/version + 完整文档 + Bento Showcase 契约达标"
        self.record("10. Gold 质量契约", not issues, detail)

    # ---------- 运行 ----------
    def evaluate(self) -> list[tuple[str, bool, str]]:
        """Run checks without printing, for delivery orchestration and API consumers."""
        self.results = []
        if not self.css_files:
            self.record("0. 输入", False, f"{self.dir} 下未找到 src/*.css")
            return self.results
        if not self.js_files:
            self.record("0. 输入", False, f"{self.dir} 下未找到 src/*.js")
            return self.results
        self.check_naming()
        self.check_vars()
        ...
        if self.quality_profile == "gold":
            self.check_gold_quality()
        return self.results
    ...
    def run(self) -> int:
        ...
        for name, ok, detail in results:
            ...
            if ok:
                ...
                failed += 1
        print(f"\n结果: {passed} 通过 / {failed} 失败 / 共 {len(results)} 项")
