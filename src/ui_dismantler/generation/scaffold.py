"""Fast manifest-to-library scaffold generator.

This is a speed aid, not a final-quality generator. It restores the deterministic
0a757f scaffold path so an agent can start from mount/create/A11y/responsive code
and spend its time on fidelity revisions instead of rewriting boilerplate.
"""
...
except ImportError as exc:  # pragma: no cover - exercised through CLI dependency errors
...
def build_placeholder_options(manifest: dict, lib_name: str) -> dict:
    """Build shape-compatible placeholder data for ``examples/template.html``."""
    structure = manifest.get("structure", {})
    views = structure.get("views", [])
    view_types = {view.get("type") for view in views}
    tabs = []
    for tab in structure.get("tabs", []):
        item = dict(tab)
        if item.get("id") == "members":
            item["count"] = 4
        elif item.get("id") in {"timeline", "works"}:
            item["count"] = 6
        tabs.append(item)
    options = {
        "title": "组件复用模板",
        ...
        "works": [],
        "moreFacts": [
            {"label": "词条类型", "value": "替换为业务条目类型", "full": False},
            {"label": "资料声明", "value": "替换为事实边界与来源声明", "full": True},
        ],
    }
    if "member-grid" in view_types:
        for index in range(4):
            letter = chr(ord("A") + index)
            options["members"].append({
                "key": f"member-{letter.lower()}",
                "name": f"示例成员 {letter}",
                "role": "示例角色",
                "shortName": letter,
                "state": "示例状态",
    ...
    if "timeline" in view_types:
        for index in range(6):
            number = index + 1
            options["timeline"].append({
                "time": f"阶段 {number}",
                "title": f"示例节点 {number}",
                "desc": "替换为节点摘要",
                "story": "替换为节点背景故事与来源说明",
                "img": "",
                "alt": f"示例节点 {number}",
            })
    if "carousel-3d" in view_types:
        for index in range(6):
            number = index + 1
            options["works"].append({
                "img": "",
                "alt": f"示例作品 {number}",
                "year": f"阶段 {number}",
                "title": f"示例作品 {number}",
...
def render_all(manifest: dict, out_dir: Path, lib_name: str, prefix: str) -> None:
    if Environment is None or FileSystemLoader is None:
        raise RuntimeError("jinja2 未安装；请先安装 jinja2") from _JINJA_IMPORT_ERROR
    env = Environment(
    ...
    # 4. template.html: same component structure with shape-compatible placeholder data.
    template_ctx = dict(ctx)
    template_manifest = dict(manifest)
    template_meta = dict(manifest.get("meta", {}))
    template_meta["title"] = "组件复用模板"
    template_manifest["meta"] = template_meta
    template_ctx["manifest"] = template_manifest
    template_ctx["options_json"] = json.dumps(
        build_placeholder_options(manifest, lib_name), ensure_ascii=False, indent=2,
    )
    template = env.get_template("example.html.j2").render(**template_ctx)
    template = template.replace(
        '<div id="mount"></div>',
        '<!-- 替换下方 mount options 即可复用；不要在可见 DOM 中硬编码业务数据。 -->\n<div id="mount"></div>',
        1,
    )
    (examples_dir / "template.html").write_text(template, encoding="utf-8")
