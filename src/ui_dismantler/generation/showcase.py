"""Showcase generator: design-token showcase page from component library CSS.

从组件库 CSS 自动提取设计令牌和组件类，生成自包含 HTML 展示页。
与组件库永远同步，零维护。

业务逻辑层：本模块提供 ``generate_showcase()`` 等函数，不含 CLI 入口。
CLI 入口见 ``ui_dismantler.cli.generate_showcase``。
"""
from __future__ import annotations
import re
from pathlib import Path

from ui_dismantler.core.common import extract_root_vars, split_media_blocks, extract_gradients, parse_rules
