...
业务逻辑层：本模块提供对比器函数，不含 CLI 入口。
CLI 入口见 ``ui_dismantler.cli.roundtrip``。
"""
...
from ui_dismantler.evaluation.scenario_coverage import compute_interaction_coverage
from ui_dismantler.paths import PROJECT_ROOT, ROUNDTRIP_RENDERER

# 默认组件库搜索根（保留原行为：相对仓库根的 src/skill）
SKILL_DIR_DEFAULT = PROJECT_ROOT / "src" / "skill"
RENDERER = ROUNDTRIP_RENDERER
