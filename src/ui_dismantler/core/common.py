"""Core utilities: CSS parsing, color parsing, variable normalization.

提供 CSS 解析、颜色解析、变量归一化等基础能力，被 analysis / generation /
validation 等领域层复用。不依赖第三方包（bs4 仅在 analysis 中用）。

旧入口 ``src/skill/scripts/_common.py`` 通过 ``_bootstrap.py`` 桥接到本模块。
"""
