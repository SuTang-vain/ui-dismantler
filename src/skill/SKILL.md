...
- `node --check` 无语法错误
- `roundtrip.py` 综合分 ≥ 0.85（结构 ≥ 0.7，文本 ≥ 0.8）
...
- **roundtrip 结构分低**：看 `structure.node_match_rate`（tag 相同节点占比）和 `class_match_rate`（类相似度），对照 `matched_nodes`/`ref_nodes` 比例。常见原因：缺视图层（如没渲染 timeline/works）、嵌套层级不对。对比器已对 sg- 前缀和类名重命名做容错匹配，不必追求类名完全一致
