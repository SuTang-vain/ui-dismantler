...
from generate_lib import render_all  # noqa: E402
...
def main():
    ap = argparse.ArgumentParser(description="垂类聚合引擎：多案例 → 垂类公共库 + 变体配置")
    ap.add_argument("vertical_dir", help="垂类目录（含若干 <案例>/index.html）")
    ap.add_argument("--out", "-o", required=True, help="输出目录")
    ap.add_argument("--name", default="", help="库名（默认从垂类目录名推断）")
    args = ap.parse_args()

    vdir = Path(args.vertical_dir).resolve()
    if not vdir.is_dir():
        print(f"ERROR: {vdir} 不是目录", file=sys.stderr)
        sys.exit(1)
    lib_name = slugify(args.name or vdir.name) or "vertical-lib"
    aggregate(vdir, Path(args.out).resolve(), lib_name)
