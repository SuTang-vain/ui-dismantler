...
from _common import safe_json_dump, slugify  # noqa: E402
...
def main():
    ap = argparse.ArgumentParser(description="manifest.json → 组件库 生成引擎")
    ap.add_argument("manifest", help="manifest.json 路径")
    ap.add_argument("--out", "-o", required=True, help="输出目录")
    ap.add_argument("--name", help="库名（默认从 manifest.caseName 推断）")
    ap.add_argument("--prefix", default="sg", help="CSS/JS 前缀（默认 sg）")
    args = ap.parse_args()
    ...
    lib_name = args.name or manifest.get("meta", {}).get("caseName") or "component-lib"
    lib_name = slugify(lib_name) or "component-lib"
