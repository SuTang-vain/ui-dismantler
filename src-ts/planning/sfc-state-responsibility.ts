import { eraseTypeScriptSyntax, parseJavaScriptOrTypeScriptErased } from "../core/ast/typescript-erasure.js";
...
export function eraseTypeScriptStateSyntax(source: string): string {
  return eraseTypeScriptSyntax(source);
}
...
export function parseTypeScriptErasedProgram(script: string): { program: any; source: string; mode: "javascript" | "typescript-erasure"; reviewReasons: string[] } {
  const parsed = parseJavaScriptOrTypeScriptErased(script);
  return {
    ...parsed,
    reviewReasons: parsed.mode === "typescript-erasure"
      ? ["TypeScript syntax was erased with line-preserving structural rules before state responsibility analysis"]
      : [],
  };
}
