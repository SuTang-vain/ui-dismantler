...
import { contributesInteractionComplexity, requiresInteractionOwner } from "../analysis/interaction-responsibility.js";
...
function estimatedComplexity(view: AnalyzedView, interactions: Interaction[], manifest: Manifest, localDataCount = 0): ComponentPlan["complexity"] {
  ...
  const interactionKinds = new Set(interactions.filter(contributesInteractionComplexity).map((item) => {
...
export function planComponents(manifest: Manifest, options: ComponentPlanningOptions = {}): ComponentPlanningReport {
  ...
  const components = views.map((view, index): ComponentPlan => {
    ...
    const complexityDependencies = [...new Set(interactions.filter(contributesInteractionComplexity).flatMap((item) => item.dataDependencies ?? []))];
  ...
  const unowned = manifest.interactions.filter((interaction) => interaction.source !== "event-listener" && requiresInteractionOwner(interaction) && !ownedInteractions.has(interaction.fingerprint));
  for (const interaction of manifest.interactions) if (!requiresInteractionOwner(interaction) && !ownedInteractions.has(interaction.fingerprint)) ownedInteractions.add(interaction.fingerprint);
