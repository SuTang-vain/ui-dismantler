...
test("frozen Vue XS Admin Data Surface Manifest is portable, deterministic, and review-gated", () => {
  ...
  assert.equal(manifest.metrics.surfaces, 4);
  assert.equal(manifest.metrics.apiSurfaces, 1);
  assert.equal(manifest.metrics.staticSurfaces, 3);
  assert.equal(manifest.metrics.reviewedFixtures, 1);
  assert.equal(manifest.metrics.fields, 14);
  assert.equal(manifest.metrics.unresolved, 12);
  assert.equal(manifest.review?.blockers.length, 12);
  assert.equal(manifest.review?.policyNotices.length, 11);
